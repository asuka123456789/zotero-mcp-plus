...
export async function testMCPIntegration(): Promise<{
  ...
  await runTest(
    ...
    async () => {
      ...
      if (response.status === 202 && response.body === "") {
        ...
        throw new Error(
          `Expected 202 with empty body, got status=${response.status}, bodyLength=${response.body.length}`,
        );
      }
    },
    tests,
  ...
  await runTest(
    "Invalid Request - Batch not supported",
    async () => {
      const { StreamableMCPServer } = await import("./streamableMCPServer");
      const mcpServer = new StreamableMCPServer();
      ...
      if (payload.error?.code === -32600 && payload.id === null) {
        ...
        throw new Error(
          `Expected batch rejection -32600 with id=null, got: ${response.body}`,
        );
      }
    },
    tests,
