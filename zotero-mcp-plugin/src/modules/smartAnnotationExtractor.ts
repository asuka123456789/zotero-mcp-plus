...
export class SmartAnnotationExtractor {
  ...
  async getAnnotations(params: {
    ...
    try {
      ...
      if (params.annotationId) {
        annotations = await this.getById(
          params.annotationId,
          options.libraryID,
        );
      } else if (params.annotationIds) {
        annotations = await this.getByIds(
  ...
  private async getById(
    ...
    const annotation = await this.annotationService.getAnnotationById(
      annotationId,
      libraryID,
    );
  ...
  private async getByIds(
    ...
    return await this.annotationService.getAnnotationsByIds(
      annotationIds,
      libraryID,
    );
  }
  ...
  private async getByItem(
    ...
    if (options.types!.includes("note")) {
      try {
        const notes = await this.annotationService.getAllNotes(
          itemKey,
          options.libraryID,
        );
    ...
    if (options.types!.some((type) => pdfTypes.includes(type))) {
      try {
        const pdfAnnotations = await this.annotationService.getPDFAnnotations(
          itemKey,
          options.libraryID,
        );
