...
import {
  initializePlusRuntime,
  suspendPlusRuntime,
  shutdownPlusRuntime,
} from "./modules/plusRuntime.ts";
...
async function processPendingAutoUpdates() {
  ...
  if (semanticEnabled !== true) return;
...
async function onStartup() {
  ...
  try {
    await initializePlusRuntime(
      () =>
        !isShuttingDown &&
        httpServer.isServerRunning() &&
        serverPreferences.isServerEnabled() &&
        Zotero.Prefs.get(PREF_WRITE_ENABLED, true) === true,
    );
  } catch {
    // 保留故障账本；只读 HTTP 能力独立于写入队列启动。
    ztoolkit.log(
      "[MCP Plus] 任务账本未就绪，文库写入已停止；没有删除或重建数据库",
      "error",
    );
  }
  ...
  serverPreferences.addObserver(async (name) => {
    ...
    if (
      name === PREF_AUTH_TOKEN ||
      name === PREF_PORT ||
      (name === PREF_ENABLED && !serverPreferences.isServerEnabled()) ||
      (name === PREF_WRITE_ENABLED &&
        Zotero.Prefs.get(PREF_WRITE_ENABLED, true) !== true)
    ) {
      try {
        await suspendPlusRuntime("security_preferences_changed");
      } catch {
        ztoolkit.log(
          "[MCP Plus] 暂停队列时账本不可写，后续投递已停止",
          "error",
        );
      }
    }

    if (name === PREF_PORT || name === PREF_ENABLED) {
      try {
...
async function onShutdown(): Promise<void> {
  ...
  try {
    await shutdownPlusRuntime();
  } catch {
    ztoolkit.log(
      "[MCP Plus] 队列关闭未完整完成；保留持久执行意图，重启后只对账不重试",
      "error",
    );
  }
  ...
  try {
    ...
    const err = error instanceof Error ? error : new Error(String(error));
    ztoolkit.log(
      `[MCP Plugin] Error flushing notifier queues: ${err.message}`,
      "error",
    );
  }
  ...
  try {
    ...
    const { getSemanticSearchService, resetSemanticSearchService } =
      await import("./modules/semantic/semanticSearchService");
    const semanticService = getSemanticSearchService();
  ...
  try {
    ...
    const { getEmbeddingService } = await import(
      "./modules/semantic/embeddingService"
    );
  ...
  try {
    ...
    const { resetVectorStore } = await import("./modules/semantic/vectorStore");
          ...
          trackedSetTimeout(() => {
            try {
              ...
              const enabledElement = doc?.querySelector(
                "#zotero-prefpane-zotero-mcp-plus-mcp-server-enabled",
              );
              const portElement = doc?.querySelector(
                "#zotero-prefpane-zotero-mcp-plus-mcp-server-port",
              );
...
function checkFirstInstallation() {
  try {
    const promptPref =
      "extensions.zotero.zotero-mcp-plus.firstInstallPromptShown";
    const hasShownPrompt = Zotero.Prefs.get(promptPref, true);
    if (!hasShownPrompt) {
      // Mark as shown immediately to prevent multiple prompts
      Zotero.Prefs.set(promptPref, true, true);
...
function showFirstInstallPrompt() {
  try {
    ...
    const title = "Zotero MCP Plus";
    const promptText =
      "Plus 与原版独立运行，文库写入和语义自动索引默认关闭。请在设置中复制带本地 Bearer token 的客户端配置；不要公开该配置。现在打开设置？";
...
const MCP_MENU_ELEMENT_IDS = [
  "zotero-mcp-plus-semantic-separator",
  "zotero-mcp-plus-semantic-menu",
  "zotero-mcp-plus-collection-semantic-separator",
  "zotero-mcp-plus-collection-semantic-menu",
];
...
function registerSemanticIndexMenu(win: _ZoteroTypes.MainWindow) {
  ...
  try {
    ...
    separator.id = "zotero-mcp-plus-semantic-separator";

    // Create parent menu
    const parentMenu = doc.createXULElement("menu");
    parentMenu.id = "zotero-mcp-plus-semantic-menu";
    ...
    popup.id = "zotero-mcp-plus-semantic-popup";

    // Create "Index Selected Items" menu item
    const indexSelectedItem = doc.createXULElement("menuitem");
    indexSelectedItem.id = "zotero-mcp-plus-index-selected";
    ...
    indexAllItem.id = "zotero-mcp-plus-index-all";
    ...
    clearSelectedItem.id = "zotero-mcp-plus-clear-selected";
...
function registerCollectionSemanticIndexMenu(win: _ZoteroTypes.MainWindow) {
  try {
    ...
    // Create menu separator
    const separator = doc.createXULElement("menuseparator");
    separator.id = "zotero-mcp-plus-collection-semantic-separator";

    // Create parent menu
    const parentMenu = doc.createXULElement("menu");
    parentMenu.id = "zotero-mcp-plus-collection-semantic-menu";
    parentMenu.setAttribute(
      "label",
    ...
    rebuildIndexItem.id = "zotero-mcp-plus-collection-rebuild-index";
    ...
    clearIndexItem.id = "zotero-mcp-plus-collection-clear-index";
